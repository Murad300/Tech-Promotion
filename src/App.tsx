...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'dashboard' | 'active-leads' | 'recycle-bin'>('dashboard');
  ...
  const handlePermanentDelete = async (id: string, detail: string) => {
    ...
    try {
      if (db && !id.startsWith('local-')) {
        ...
        showNotice("সার্ভার থেকে চিরতরে মুছে ফেলা হয়েছে!", "success");
      } else {
        setContacts(prev => prev.filter(c => c.id !== id));
        addLog(`Erased local contact: ${detail}`);
        showNotice("চিরতরে মুছে ফেলা হয়েছে!", "success");
      }
    } catch (err: any) {
      addLog(`Failed to permanently delete contact: ${err.message}`);
      showNotice(`মুছে ফেলতে ব্যর্থ হয়েছে: ${err.message}`, "error");
    }
  };
  ...
  const handleSendImo = () => {
    ...
    showNotice("IMO মেসেজ ক্লিপবোর্ডে কপি করা হয়েছে! দয়া করে অ্যাপ খুলে পেস্ট করুন।", "info");
  ...
  return (
    <div id="dashboard-layout" className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans select-none pb-8">
      ...
      <section className="bg-slate-900 text-white px-4 py-3 shrink-0 grid grid-cols-4 gap-1.5 border-b border-slate-800">
        <div className="bg-slate-950/40 border border-indigo-500/20 rounded-xl p-2 text-center flex flex-col justify-center">
          <span className="text-[7px] sm:text-[8px] font-bold text-indigo-400 uppercase tracking-widest block">Active Phones</span>
          <div className="flex items-center justify-center gap-1 mt-0.5">
            <Phone className="w-3 h-3 text-indigo-400" />
            <span className="text-sm font-black text-white">{activePhoneCount}</span>
          </div>
        </div>

        <div className="bg-slate-950/40 border border-emerald-500/20 rounded-xl p-2 text-center flex flex-col justify-center">
          <span className="text-[7px] sm:text-[8px] font-bold text-emerald-400 uppercase tracking-widest block">Active Emails</span>
          <div className="flex items-center justify-center gap-1 mt-0.5">
            <Mail className="w-3 h-3 text-emerald-400" />
            <span className="text-sm font-black text-white">{activeEmailCount}</span>
          </div>
        </div>

        <div className="bg-slate-950/40 border border-rose-500/20 rounded-xl p-2 text-center flex flex-col justify-center">
          <span className="text-[7px] sm:text-[8px] font-bold text-rose-400 uppercase tracking-widest block">Recycled Phones</span>
          <div className="flex items-center justify-center gap-1 mt-0.5">
            <Trash2 className="w-3 h-3 text-rose-400" />
            <span className="text-sm font-black text-white">{recycledPhones.length + deepRecycledPhones.length}</span>
          </div>
        </div>

        <div className="bg-slate-950/40 border border-amber-500/20 rounded-xl p-2 text-center flex flex-col justify-center">
          <span className="text-[7px] sm:text-[8px] font-bold text-amber-400 uppercase tracking-widest block">Recycled Emails</span>
          <div className="flex items-center justify-center gap-1 mt-0.5">
            <Trash2 className="w-3 h-3 text-amber-400" />
            <span className="text-sm font-black text-white">{recycledEmails.length + deepRecycledEmails.length}</span>
          </div>
        </div>
      </section>
        ...
        <section className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-4 space-y-3.5">
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 text-xs font-extrabold py-3.5 rounded-xl transition-all cursor-pointer shadow-md ${
                selectedContact && selectedContact.phoneNumber
                  ? 'bg-sky-500 hover:bg-sky-600 active:scale-95 text-white ring-2 ring-sky-400/50 shadow-lg shadow-sky-500/30'
                  : 'bg-slate-100 text-slate-400 opacity-60 cursor-not-allowed border border-slate-200'
              }`}
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 text-xs font-extrabold py-3.5 rounded-xl transition-all cursor-pointer shadow-md ${
                selectedContact && selectedContact.phoneNumber
                  ? 'bg-blue-600 hover:bg-blue-700 active:scale-95 text-white ring-2 ring-blue-400/50 shadow-lg shadow-blue-500/30'
                  : 'bg-slate-100 text-slate-400 opacity-60 cursor-not-allowed border border-slate-200'
              }`}
              title="Imo Promotion"
            >
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 text-xs font-extrabold py-3.5 rounded-xl transition-all cursor-pointer shadow-md ${
                selectedContact && selectedContact.phoneNumber
                  ? 'bg-amber-500 hover:bg-amber-600 active:scale-95 text-white ring-2 ring-amber-400/50 shadow-lg shadow-amber-500/30'
                  : 'bg-slate-100 text-slate-400 opacity-60 cursor-not-allowed border border-slate-200'
              }`}
              title="SMS Promotion"
            >
            ...
            <button
              ...
              className={`col-span-2 flex items-center justify-center gap-1.5 text-xs font-extrabold py-3.5 rounded-xl transition-all cursor-pointer shadow-md ${
